...
let response = await fetch("http://localhost:8080/sectors/all")
...
let cardGroupSmartRef = document.getElementById("cardgroupsmart")

data.forEach((d)=>{
    let randomImage = parseInt(Math.random()*100)
    let innerElement = ` <div class="col">
          <div class="card h-100">
            <img src="https://picsum.photos/200?random=${randomImage}" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">${d.sector_id}</h5>
              <p class="card-text">${d.sector_name}</p>
            </div>
          </div>
        </div>`

        //create new element using innerElement
       
        cardGroupSmartRef.innerHTML += innerElement
})
